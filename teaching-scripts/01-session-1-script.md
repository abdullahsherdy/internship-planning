...
**Session outcome:** Every trainee runs a controller-based ASP.NET Core API, hits `GET /api/health` and `GET /api/projects`, sets a breakpoint, and explains the request/response cycle in their own words.

---

## Before the Session (Instructor Prep, ~30 min)

- [ ] Instructor repo at tag `session-1-start` (empty) and `session-1-done` (finished code below).
- [ ] `dotnet --version` shows .NET 10 SDK on your machine and on the shared known-issues sheet.
- [ ] `.http` file ready (code in section E below).
- [ ] Pre-work submissions reviewed; note who failed the setup gate — pair them with a verified-setup trainee.
- [ ] Retrieval quiz loaded in your polling tool (questions below).
- [ ] Second monitor: this script + a timer. Do not improvise timings in Session 1.

**Known-issues sheet (have answers ready):**

| Symptom | Fix |
|---|---|
| `dotnet` not recognized | SDK not installed or PATH; reinstall, restart terminal |
| HTTPS dev-cert warning | `dotnet dev-certs https --trust` |
| Port already in use | Change port in `launchSettings.json` or kill the process |
| Browser shows nothing on `/` | Expected — no root route; use `/api/health` |
...
## Instructor Post-Session (15 min)

- [ ] Tag your live-coded repo `session-1-live` and push (trainees compare against it).
- [ ] Read exit tickets; anything fuzzy for >30% becomes the first 5 minutes of Session 2's concept block.
- [ ] Sort checkpoint sentence-answers into pass / follow-up; send office-hours invites same day.
- [ ] Update the known-issues sheet with any new environment failure seen live.
- [ ] Adjust Session 2 pairing from observed (not self-reported) ability.

---

## Script Pattern Reference (for Sessions 2-8)

Every session script follows this template — keep it when writing the next ones:

1. **Prep checklist** with known-issues table.
2. **Retrieval quiz** — 5 questions, previous session + pre-work; read the room, don't grade.
3. **Concept block** — each concept as *problem framing → solution → check question → scripted transition line*; no framework names before the mental model.
4. **Live coding** — typed not pasted, one **scripted deliberate error** feeding the session's AI practice (Explain → Verify → Fix → Rebuild), and one **payoff pain** that motivates a future session.
5. **Guided pairs** — checklist exercise, separate foundation hints channel, stretch for early finishers, instructor rotates with pointed questions.
6. **Independent checkpoint** — small individual change + a written explanation; the writing is the assessment.
7. **Wrap** — cold-call recap against the session diagram, exit ticket, homework with an explicit "explained later, used today" boundary.
